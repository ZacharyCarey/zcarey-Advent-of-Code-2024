...
{
    // Custom priority quueue from the internet that supports priority changing
    ...
    public class CustomPriorityQueue<T>
    {
        ...
        public CustomPriorityQueue(bool isMinPriorityQueue = false)
        {
        ...
        /// <param name="obj"></param>
        /// <param name="priority"></param>
        public void UpdatePriority(T obj, int priority, Func<T, T, bool> equalityComparer)
        {
            int i = 0;
            for (; i <= heapSize; i++)
            {
                Node node = queue[i];
                if (equalityComparer(node.Object, obj))
                {
